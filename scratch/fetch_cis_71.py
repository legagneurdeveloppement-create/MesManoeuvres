...
query = """
[out:json];
area["name"="Saône-et-Loire"]->.searchArea;
(
...
try:
    ...
    for element in data['elements']:
        ...
        if name:
            name = name.replace("Centre de secours", "").replace("Centre d'Incendie et de Secours", "").replace("C.I.S.", "").replace("CIS", "").replace("CS", "").strip(" de").strip(" d'").strip("- ").strip()
            if name:
